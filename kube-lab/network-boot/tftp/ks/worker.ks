...
# Set hostname dynamically based on MAC address
# Extract last 4 characters of the first MAC address and use it to set hostname
MAC=$(ip link | grep link/ether | head -n 1 | awk '{print $2}' | sed 's/://g' | tail -c 5)
hostnamectl set-hostname k8s-worker-$MAC
